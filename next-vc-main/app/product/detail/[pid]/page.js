...
export default function ProductDetailIdPage() {
  ...
  const getDetailData = async () => {
    try {
      const res = await fetch(
        `https://echora-kwvs.onrender.com/api/products/${pid}/${firstSkuId}`
      )
      ...
      // console.log(`https://echora-kwvs.onrender.com/api/products/${pid}/${firstSkuId}`)
  ...
  const getFav = async (uid) => {
    try {
      const res = await fetch(`https://echora-kwvs.onrender.com/api/favorite/${uid}`)
      const data = await res.json()
  ...
  const addToFav = async (uid, skuid) => {
    try {
      const res = await fetch(
        `https://echora-kwvs.onrender.com/api/favorite/${uid}/${skuid}`,
        {
          method: 'PUT',
        }
      )
  ...
  const getMayLikeData = async (colorId) => {
    try {
      const res = await fetch(
        `https://echora-kwvs.onrender.com/api/products/maylike/${colorId}`
      )
      const data = await res.json()
            ...
            <div className="row h-100">
              ...
              <div className="g-pd-discrip col-lg-5 h-100">
                <button
                  className="g-heart"
                  onClick={toggleFav}
                  aria-label="Toggle favorite" // 為無障礙提供描述
                  style={{ background: 'none', border: 'none', padding: 0 }} // 這樣可以讓按鈕的樣式保持原來的圖片顯示
                >
                  <img
                    src={`/images/product/detail/${favIcon}`}
                    alt="Favorite icon"
                    style={{ pointerEvents: 'none' }} // 禁用 img 元素的直接點擊事件，讓它繼承按鈕的點擊行為
                  />
                </button>
                ...
                <button
                  ...
                  disabled={
                    detailData[0]?.stock[selectedSku] > 0 ? false : true
                  }
                  onClick={() => {
